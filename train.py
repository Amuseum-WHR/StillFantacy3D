...
def validate(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS, relight= None):
    ...
    rast_dir = os.path.join(out_dir, "rast")
    ...
    for it, target in enumerate(dataloader_validate):
        ...
        for k in result_dict.keys():
            ...
            if k == 'shaded':
                ...
                np.save(rast_dir + '/' + ('val_%06d_%s.npy' % (it, k)), np_img)
...
@torch.no_grad()     
def validate_roll(glctx, geometry, opt_material, lgt, dataset_validate, out_dir, FLAGS, target_it):
    ...
    for it, target in enumerate(dataloader_validate):
        if it == target_it:
            # Mix validation background
            target = prepare_batch(target, 'white')
            for itr in range(nums_image):
                relight = light.load_env(FLAGS.relight, scale=FLAGS.env_scale, random_rotate=itr/nums_image)
                relight.build_mips()
                if FLAGS.camera_space_light:
                    relight.xfm(target['mv'])
                result_image, result_dict = validate_itr_roll(glctx, target, geometry, opt_material, relight, FLAGS)
                for k in result_dict.keys():
                    np_img = result_dict[k].detach().cpu().numpy()
                    if k == 'relight':
                        util.save_image(relight_dir + '/' + ('val_%06d_%s.png' % (itr, k)), np_img)
            break
    if 'relight' in result_dict.keys():
        save_gif(relight_dir,30)
    return 0

def validate_itr_roll(glctx, target, geometry, opt_material, lgt, FLAGS, idx=None):
    result_dict = {}
    with torch.no_grad():
        buffers = geometry.render(glctx, target, lgt, opt_material, if_use_bump = FLAGS.if_use_bump)
        result_dict['relight'] = buffers['shaded'][0, ..., 0:3]
        result_dict['relight'] = util.rgb_to_srgb(result_dict['relight'])
        result_image = result_dict['relight'] 
...
class Trainer(torch.nn.Module):
    ...
    def load(self, it=None):
        if it is None:
            # print(os.path.join(self.FLAGS.out_dir, "checkpoints", "*.pth"))
            # for x in glob.glob(os.path.join(self.FLAGS.out_dir, "checkpoints", "*.pth")):
...
if __name__ == "__main__":
    ...
    if FLAGS.mode == 'geometry_modeling' :   
        ...
        base_mesh = mesh.load_mesh(FLAGS.base_mesh, mtl_override=FLAGS.base_mesh[:-3] + 'mtl')
    ...
    elif FLAGS.mode == 'rotate':
        base_mesh = mesh.load_mesh(FLAGS.base_mesh)
        geometry = DLMesh(base_mesh, FLAGS)
        mat = initial_guness_material(geometry, True, FLAGS)
        model = Trainer(glctx, geometry, lgt, mat, optimize_geometry=False, optimize_light=False, FLAGS=FLAGS, guidance=None)
        load_it = model.load() # load latest model
        validate_roll(glctx, geometry, mat, lgt, dataset_gif, os.path.join(FLAGS.out_dir, "validate_relight_{}".format(FLAGS.target_it)), FLAGS, FLAGS.target_it)
