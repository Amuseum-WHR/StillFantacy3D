...
class DLMesh(torch.nn.Module):
    def __init__(self, initial_guess, FLAGS):
        ...
        if 'mask_3d' in FLAGS:
            self.mask3d        = torch.from_numpy(np.load(FLAGS.mask_3d)).to(self.mesh.v_pos.device)
            self.fixed_mtl     = FLAGS.fixed_mtl
        else:
            self.mask3d        = None
            self.fixed_mtl     = None
    ...
    def render(self, glctx, target, lgt, opt_material, bsdf=None,if_normal=False, mode = 'appearance_modeling', if_flip_the_normal = False, if_use_bump = False, it = None):
    ...
    def tick(self, glctx, target, lgt, opt_material, iteration, if_normal, guidance,  mode, if_flip_the_normal, if_use_bump):
        ...
        # ==============================================================================================
        buffers= self.render(glctx, target, lgt, opt_material, if_normal = if_normal, mode = mode,  if_flip_the_normal = if_flip_the_normal, if_use_bump = if_use_bump, it = iteration)
