...
def shade(
    ...
    if mask_list != None:
        if "ks" in fixed_mtl or "m" in fixed_mtl:
            new_ks = torch.zeros_like(ks)
        if "kd" in fixed_mtl:
            new_kd = torch.zeros_like(kd)
        for msk in mask_list:
            if "ks" in fixed_mtl:
                new_ks[...,0:1] += msk * (torch.sum(ks[...,0:1] * msk, dim=(1,2,3)) / torch.sum(msk, dim=(1,2,3))).reshape(-1,1,1,1)
                new_ks[...,1:2] += msk * (torch.sum(ks[...,1:2] * msk, dim=(1,2,3)) / torch.sum(msk, dim=(1,2,3))).reshape(-1,1,1,1)
                new_ks[...,2:3] += msk * (torch.sum(ks[...,2:3] * msk, dim=(1,2,3)) / torch.sum(msk, dim=(1,2,3))).reshape(-1,1,1,1)
            if "kd" in fixed_mtl:
                new_kd[...,0:1] += msk * (torch.sum(kd[...,0:1] * msk, dim=(1,2,3)) / torch.sum(msk, dim=(1,2,3))).reshape(-1,1,1,1)
                new_kd[...,1:2] += msk * (torch.sum(kd[...,1:2] * msk, dim=(1,2,3)) / torch.sum(msk, dim=(1,2,3))).reshape(-1,1,1,1)
                new_kd[...,2:3] += msk * (torch.sum(kd[...,2:3] * msk, dim=(1,2,3)) / torch.sum(msk, dim=(1,2,3))).reshape(-1,1,1,1)
        if "m" in fixed_mtl:
            if "ks" not in fixed_mtl:
                new_ks[...,0:1] = ks[...,0:1]
                new_ks[...,1:2] = ks[...,1:2]
            new_ks[...,2:3] = mask_list[0] * 0.9 + mask_list[1] * 0.15 # 0.25
            # new_ks[...,2:3] += msk * (ks[...,2:3]-(torch.sum(ks[...,2:3] * msk, dim=(1,2,3)) 
            #                                        / torch.sum(msk, dim=(1,2,3))).reshape(-1,1,1,1))
        if "ks" in fixed_mtl or "m" in fixed_mtl:
            ks = new_ks
        if "kd" in fixed_mtl:
            kd = new_kd
    ...
    mask = torch.stack(mask_list) if mask_list is not None else None
    buffers = {
...
def render_layer(
    ...
    if mask_3d != None:
        mask_list = []
        num_classes = torch.max(mask_3d)
        face_id = torch.tensor(rast_out_s[...,-1:].detach().clone().reshape(-1), dtype=torch.int64) - 1
        rast_class = mask_3d[face_id].clone()
        for idx in range(1, num_classes+1):
            mask = torch.tensor(rast_class == idx)
            mask[face_id < -0.5] = 0
            mask = mask.reshape(rast_out_s[...,-1:].shape)
    ...
    buffers = shade(gb_pos, gb_geometric_normal, gb_normal, gb_tangent, gb_texc, gb_texc_deriv, 
        view_pos, lgt, mesh.material, bsdf,if_normal,normal_rotate, mode, if_flip_the_normal, if_use_bump, it, mask_list, fixed_mtl)
...
def render_mesh(
    ...
    with dr.DepthPeeler(ctx, v_pos_clip, mesh.t_pos_idx.int(), full_res) as peeler:
        for _ in range(num_layers):
            ...
            layers += [(render_layer(rast, db, mesh, view_pos, lgt, resolution, spp, msaa, bsdf, if_normal, normal_rotate, mode, if_flip_the_normal, if_use_bump, it, mask_3d, fixed_mtl), rast)]
    # Setup background
    ...
    for key in layers[0][0].keys():
        if key == 'mask':
            out_buffers[key] = layers[0][0][key]
            continue
        elif key == 'shaded':
